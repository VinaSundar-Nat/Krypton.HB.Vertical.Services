...
public class BaseStepDefinition : IClassFixture<TestWebApplicationFactory<Program>>
{
    ...
    private async Task<(HttpStatusCode code, string response)> setup(ScenarioContext scenarioContext)
    {
        ...
        return (title switch
        {
            "core1" => new(HttpStatusCode.OK, await GetContent("core_200_des_res.json", "Responses")),
    ...
    protected void addHeaders(HttpRequestMessage request)
    {
        ...
        request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/vnd.ms.krchat.1.0+json"));
    ...
    [AfterScenario]
    public async Task Destroy(ScenarioContext scenarioContext)
    {
        _Client.Dispose();
        _Client = null;
